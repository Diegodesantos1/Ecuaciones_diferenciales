...
class Ecuaciones:
    def ecuacion1():
        ...
        print(f" La solución de la primera ecuación es {sp.pprint(sol)}")
    def ecuacion2():
        x = sp.Symbol("x") ; y = sp.Function("y")
        ecuacion = sp.Eq(y(x).diff(x) * sp.sin(x), y(x) * sp.log(y(x)))
        ics = {y((math.pi/2)): math.e} ; sol = sp.dsolve(ecuacion, y(x), ics=ics) ;   sp.pprint(sol)
        print(f" La solución de la segunda ecuación es: {sp.pprint(sol)}")
    def ecuacion3():
        t = sp.Symbol("t") ; y = sp.Function("y")
        ecuacion = sp.Eq(y(t).diff(t) - (y(t)/(t-2)), 2*(t-2)**2) ; sol = sp.dsolve(ecuacion, y(t))
        print(f" La solución de la tercera ecuación es {sp.pprint(sol)}")
    def ecuacion4():
        t = sp.Symbol("t") ; y = sp.Function("y")
        ecuacion = sp.Eq(2*t*y(t).diff(t) - y(t), 3*t**2) ; sol = sp.dsolve(ecuacion, y(t))
        print(f" La solución de la cuarta ecuación es {sp.pprint(sol)}")
